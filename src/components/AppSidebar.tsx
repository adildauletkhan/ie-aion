  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2 min-w-0 overflow-hidden">
          ...
          {!collapsed && (
            <div className="min-w-0">
              <span className="text-xs font-bold tracking-wider text-sidebar-primary uppercase leading-tight block">
                {t("appTitle")}
              </span>
      ...
      <nav className="flex-1 py-4 space-y-1 px-2 overflow-y-auto">
        ...
        {/* ── Dynamic pack navigation (energy / mining) ── */}
        {useDynamicNav && industryPack.modules.map((mod) => {
          const IconComp = ICON_MAP[mod.icon] ?? Plug;
          return (
            <NavLink
              key={mod.route}
              to={mod.route}
        ...
        {/* ── Upstream / other industry navigation (original) ── */}
        {!useDynamicNav && (
          <>
            {/* Top items */}
            {topItems
