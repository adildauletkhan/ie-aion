...
def ai_chat(
    ...
    system_prompt = (
        ...
        "RULES: "
        "1. CRITICAL — Language detection: The user's message starts with a [LANGUAGE INSTRUCTION] block. "
        "You MUST obey it strictly. If it says respond in Kazakh — respond ONLY in Kazakh (қазақ тілінде). "
        "If it says respond in English — respond ONLY in English. "
        "If it says respond in Russian — respond ONLY in Russian. "
        "NEVER mix languages. NEVER fall back to Russian or English when Kazakh is requested. "
        "2. Use exact numbers from context. If data is missing, say clearly in the requested language. "
        "3. Never expose user data (passwords, emails). "
        "4. Give a natural conversational answer. Do NOT add section headers like "
        "'Краткий ответ', 'Детали', 'Источники', 'Short answer', 'Details', 'Sources', "
        "'Қысқаша жауап', 'Мәліметтер', 'Деректер көздері'. Just answer directly. "
